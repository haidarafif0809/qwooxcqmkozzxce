...
$pilih_akses_tombol = $db->query("SELECT tombol_submit_ugd,tombol_bayar_ugd,tombol_piutang_ugd,tombol_simpan_ugd,tombol_bayar_ugd,tombol_batal_ugd FROM otoritas_penjualan_ugd WHERE id_otoritas = '$_SESSION[otoritas_id]' ");
...
<div class="row">
  ...
  <div class="col-xs-3">
    ...
    <select type="text" style="height:15px" class="form-control " name="kode_barang" autocomplete="off" id="kode_barang" >
...
<?php endif ?>
                ...
                <span id="span_tbs" style="display: none">            
...
Radiologi  </button>
  ...
  <div align="right" class="BtnOnTop">    
          ...
          <button type="submit" id="transaksi_baru" class="btn btn-info" style="display: none"><i class="fa fa-refresh"></i> Transaksi Baru (Ctrl + M)</button>
  ...
  <div class="form-group">
    ...
    <div class="row">
         ...
         <div class="col-xs-6">
          ...
          <input type="text" name="potongan" style="height:10px;font-size:15px" id="potongan_penjualan" value="<?php echo $data_diskon['diskon_nominal']; ?>" class="form-control" placeholder="Diskon Rp" autocomplete="off"  onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);">
            
          </div>
          <div class="col-xs-6">
          <label> Diskon ( % )</label><br>
          <input type="text" name="potongan_persen" style="height:10px;font-size:15px" id="potongan_persen" value="<?php echo $data_diskon['diskon_persen']; ?>%" class="form-control" placeholder="Diskon %" autocomplete="off" >
          </div>
      ...
      data = data.replace(/\s+/g, '');
        if (data == '') {
          data = 0;
            ...
            $("#biaya_admin").val(tandaPemisahTitik(Math.round(hitung_biaya)));
...
// START DATATABLE AJAX START DATATABLE AJAX START DATATABLE AJAX START DATATABLE AJAX START DATATABLE AJAX START DATATABLE AJAX
      $('#tabel_tbs_penjualan').DataTable().destroy();
            ...
            "ajax":{
              url :"data_tbs_penjualan_ugd_input_data.php", // json datasource
               "data": function ( d ) {
                  d.no_reg = $("#no_reg").val();
...
if (kolom_cek_harga == '0') {
  ...
  else if (ber_stok == 'Jasa' || ber_stok == 'BHP' )
  {
    ...
    $("#biaya_admin").val(tandaPemisahTitik(biaya_admin));
                                ...
                                if (ppn == 'Exclude') {// if (ppn == 'Exclude')
                                  ...
                                  /*
                                     ...
                                     else if(pot_fakt_rp != 0 && pot_fakt_rp != 0)
                                    {
                                      ...
                                      $("#hapus-tbs-"+id+"").attr("data-subtotal",subtotal_tbs);
                                      $("#text-potongan-"+id+"").show();
                                      $("#input-potongan-"+id).attr("type", "hidden");
