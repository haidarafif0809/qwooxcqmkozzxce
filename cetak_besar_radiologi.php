...
include 'db.php';
  ...
  $no_faktur = $_GET['no_faktur'];
    ...
    $query0 = $db->query("SELECT p.biaya_admin,p.id,p.no_faktur,p.total,p.kode_pelanggan,p.keterangan,p.cara_bayar,p.tanggal,p.tanggal_jt,p.jam,p.user,p.sales,p.kode_meja,p.status,p.potongan,p.tax,p.sisa,p.kredit,p.kode_gudang,p.tunai,dp.satuan,dp.jumlah_barang,dp.subtotal,dp.nama_barang,dp.harga, da.nama_daftar_akun, u.nama AS kasir FROM penjualan p INNER JOIN detail_penjualan dp ON p.no_faktur = dp.no_faktur INNER JOIN user u ON p.sales = u.id  INNER JOIN daftar_akun da ON p.cara_bayar = da.kode_daftar_akun WHERE p.no_faktur = '$no_faktur' AND dp.radiologi = 'Radiologi' ORDER BY p.id DESC");
...
$row_pel = mysqli_num_rows($select_pelanggan);

if ($row_pel > 0) {
  $nama_pelanggan = $dd['nama_pelanggan'];
  $alamat_sekarang = $dd['alamat_sekarang'];
}
else{
  $nama_pelanggan = $dd['nama_pelanggan'];
  $alamat_sekarang = $dd['alamat_sekarang'];
...
<div class="container">
 ...
 <table>
  <tbody>
      ...
      <tr><td  width="25%"><font class="satu">Pasien</font></td> <td> :&nbsp;</td> <td> <font class="satu"><?php echo $nama_pelanggan; ?></font> </td></tr>
      <tr><td  width="25%"><font class="satu">Alamat</font></td> <td> :&nbsp;</td> <td><font class="satu"> <?php echo $alamat_sekarang; ?> </font></td></tr>
...
<table id="tableuser" class="table table-bordered table-sm">
        <thead>
            ...
            <th class="table1" style="width: 5%"> <center> Jenis </center> </th>
