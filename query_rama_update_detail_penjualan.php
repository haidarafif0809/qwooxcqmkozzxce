...
$select_detail_penjualan = $db->query("SELECT * FROM detail_penjualan WHERE no_faktur = '2/JL/01/17'");
while ($dp = mysqli_fetch_array($select_detail_penjualan)) {
    ...
    $insert_detail_penjualan = "INSERT INTO detail_penjualan (no_faktur, tanggal, jam, kode_barang, nama_barang, jumlah_barang, asal_satuan,satuan, harga, subtotal, potongan, tax, sisa) VALUES ('$dp[no_faktur]', '$dp[tanggal]', '$dp[jam]', '$dp[kode_barang]','$dp[nama_barang]','$dp[jumlah_barang]','$dp[asal_satuan]','$dp[satuan]','$dp[harga]','$dp[subtotal]','$dp[potongan]','$dp[tax]', '$dp[sisa]')";
