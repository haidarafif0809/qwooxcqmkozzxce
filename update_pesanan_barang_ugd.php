...
$query->execute();
    ...
    $query9 = $db->query("SELECT * FROM tbs_fee_produk WHERE no_reg = '$no_reg' ");
    while($cek9 = mysqli_fetch_array($query9))
    {

$select_fee = $db->query("SELECT jumlah_uang,jumlah_prosentase FROM fee_produk WHERE nama_petugas = '$cek9[nama_petugas]'  ");
$ff = mysqli_fetch_array($select_fee);
    ...
    $prosentase = $ff['jumlah_prosentase'];
        ...
        if ($prosentase != 0)
            ...
            $query1 = $db->query("UPDATE tbs_fee_produk SET jumlah_fee = '$fee_prosentase_produk' WHERE nama_petugas = '$nm_pet' AND kode_produk = '$kode' AND no_reg = '$no_reg'"); 
   ...
   elseif ($nominal != 0) 
            ...
            $query01 = $db->query("UPDATE tbs_fee_produk SET jumlah_fee = '$fee_nominal_produk' WHERE nama_petugas = '$nm_pet' AND kode_produk = '$kode' AND no_reg = '$no_reg' ");
