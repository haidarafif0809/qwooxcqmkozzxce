...
$query_pelanggan = $db_pasien->query("SELECT kode_pelanggan FROM pelanggan WHERE (kode_pelanggan != '0' OR kode_pelanggan IS NULL OR kode_pelanggan != '') AND kode_klinik =  '" . $kode_klinik_reg["kode_klinik"] . "' ORDER BY CONVERT(SUBSTRING(kode_pelanggan,5,99),decimal) DESC LIMIT 1 ")->fetch_array();
