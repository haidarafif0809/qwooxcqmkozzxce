...
//menampilkan seluruh data yang ada pada tabel REGISTRASI (Rawat Inap)
$select_rj = $db->query("SELECT * FROM registrasi WHERE jenis_pasien = 'Rawat Inap'");
$out_rj = mysqli_fetch_array($select_rj);
 ...
 <div class="container">
<h3><b><center>Laporan Kunjungan Pasien Rawat Inap</center></b></h3>
...
<div class="form-group"> 
	<input type="text" name="sampai_tanggal" id="sampai_tanggal" value="<?php echo date("Y-m-d"); ?>" required="" autocomplete="off" class="form-control tanggal_cari">
</div>
...
<span id="result"><!--span untuk table-->       
    <table id="table_lap_ri" class="table table-bordered table-sm">
        <thead>
...
<span id="berdasarkan_penjamin" style="display: none;"><!--span untuk table-->       
    <table id="table_lap_ri_penjamin" class="table table-bordered table">
        <thead>
          <th style='background-color: #4CAF50; color:white'> No RM </th>
          <th style='background-color: #4CAF50; color:white'> No REG </th>
          <th style='background-color: #4CAF50; color:white'> Nama Pasien </th>
...
<a href='export_excel_kunjungan_pasien_rm_ri.php' type='submit' id='btn_exc' class='btn btn-warning btn-lg'>Download Excel</a>
</span>

<span id="download_exc_tanggal">
<a href='export_excel_kunjungan_pasien_tanggal_ri.php' type='submit' id='btn_exc_tanggal'  class='btn btn-warning btn-lg'>Download Excel</a>
</span>

<span id="download_exc_rm_tanggal">
<a href='export_excel_ex_ri.php' type='submit' id='btn_exc_rm_tanggal' class='btn btn-warning btn-lg'>Download Excel</a>
</span>

<span id="download_exc_penjamin">
<a href='export_excel_ex_ri_penjamin.php' type='submit' id='btn_exc_penjamin' class='btn btn-warning btn-lg'>Download Excel</a>
</span>
...
$(document).on('click','#lihat_rm',function(e) {
        ...
        "emptyTable":   "My Custom Message On Empty Table"
          ...
          "ajax":{
            url :"datatable_lab_kunj_rm_ri.php", // json datasource
             "data": function ( d ) {
                d.no_rm = $("#no_rm").val();
  ...
  $("#btn_exc").attr("href", "export_excel_kunjungan_pasien_rm_ri.php?no_rm="+no_rm+"");
...
$(document).on('click','#lihat_penj',function(e) {
        ...
        $('#berdasarkan_penjamin').show();
         ...
         $('#table_lap_ri_penjamin').DataTable().destroy();
        ...
        if (penjamin == '') {
          ...
          var dataTable = $('#table_lap_ri_penjamin').DataTable( {
          "processing": true,
          "serverSide": true,
          "info":     false,
          "language": {
        "emptyTable":     "My Custom Message On Empty Table"
          ...
          "ajax":{
            url :"datatable_lab_kunj_ri_penjamin.php", // json datasource
             "data": function ( d ) {
                 d.penjamin    = $("#penjamin").val();
                d.dari_tanggal = $("#dari_tanggal_penj").val();
                d.sampai_tanggal = $("#sampai_tanggal_penj").val();
                // d.custom = $('#myInput').val();
                // etc
            },
                type: "post",  // method  , by default get
            error: function(){  // error handling
              $(".tbody").html("");
              $("#table_lap_ri_penjamin").append('<tbody class="tbody"><tr><th colspan="3"></th></tr></tbody>');
              $("#tableuser_processing").css("display","none");
            }
          }
      ...
      $("#download_exc_rm_tanggal").hide();
      $("#download_exc").hide();
  $("#btn_exc_penjamin").attr("href", "export_excel_ex_ri_penjamin.php?penjamin="+penjamin+"&dari_tanggal="+dari_tanggal+"&sampai_tanggal="+sampai_tanggal+"");
