...
<table id="tableuser" class="table table-bordered">
    ...
    $perintah = $db->query("SELECT b.id AS id_produk,b.satuan AS satuan_dasar, s.nama AS satuan_beli ,ss.nama AS satuan_asli, dp.no_faktur, dp.tanggal, dp.kode_barang, dp.nama_barang, dp.jumlah_barang, dp.satuan, dp.harga, dp.subtotal, dp.potongan, dp.tax, p.suplier, dp.satuan, dp.asal_satuan  FROM detail_pembelian dp LEFT JOIN pembelian p ON dp.no_faktur = p.no_faktur INNER JOIN satuan s ON dp.satuan = s.id INNER JOIN satuan ss ON dp.asal_satuan = ss.id INNER JOIN barang b ON dp.kode_barang = b.kode_barang WHERE  p.suplier = '$suplier' GROUP BY dp.no_faktur, dp.kode_barang");
    ...
    //menyimpan data sementara yang ada pada $perintah
      while ($data1 = mysqli_fetch_array($perintah))
      {
        ...
        $sisa = cekStokPerfaktur($data1['kode_barang'],$data1['no_faktur']);
     ...
     $sisa_konversi = $sisa % $data_konversi['konversi'];
...
if ($sisa_konversi == 0)
{
      ...
      echo "<tr class='pilih' data-kode='". $data1['kode_barang'] ."(". $data1['nama_barang'] .")' nama-barang='". $data1['nama_barang'] ."' satuan='". $data1['satuan'] ."' no_faktur='". $data1['no_faktur'] ."' harga='". $harga ."' jumlah-barang='". $data1['jumlah_barang'] ."' sisa='". $konversi_data ."' harga_pcs = ".$data1['harga']." id_produk='". $data1['id_produk'] ."' asal_satuan = '".$data1['asal_satuan']."'  satuan_pcs = '".$data1['satuan_dasar']."' satuan_beli = '".$data1['satuan']."'>";
        ...
        // menampilkan data
      echo "<tr class='pilih' data-kode='". $data1['kode_barang'] ."(". $data1['nama_barang'] .")' nama-barang='". $data1['nama_barang'] ."' satuan='". $data1['asal_satuan'] ."' no_faktur='". $data1['no_faktur'] ."' harga='". $data1['harga'] ."' jumlah-barang='". $data1['jumlah_barang'] ."' sisa='". $sisa ."' id_produk='". $data1['id_produk'] ."' asal_satuan = '".$data1['asal_satuan']."' harga_pcs = '".$data1['harga']."'  satuan_pcs = '".$data1['satuan_dasar']."' satuan_beli = '".$data1['satuan']."'>";
      ...
      {
        echo"<td>". $sisa ." ". $data1['satuan_asli'] ."</td>";
      }
