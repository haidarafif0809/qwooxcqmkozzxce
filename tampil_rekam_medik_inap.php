...
switch ($cari_berdasarkan) {
    case "nama":
        $query1= $db->query("SELECT * FROM rekam_medik_inap WHERE nama LIKE '%$pencarian%' AND tanggal_periksa >= '$dari_tanggal' AND tanggal_periksa <= '$sampai_tanggal' ");
...
tr:nth-child(even){background-color: #f2f2f2}
  ...
  echo"
        ...
        break;
    case "no_rm":
        $query2= $db->query("SELECT * FROM rekam_medik_inap  WHERE no_rm LIKE '%$pencarian%' AND tanggal_periksa >= '$dari_tanggal' AND tanggal_periksa <= '$sampai_tanggal'");
        echo"<table id='table-group' class='table table-bordered table-sm'>
