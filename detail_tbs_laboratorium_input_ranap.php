...
$no_reg = $_POST['no_reg'];
$pemeriksaan_laboratorium = $_POST['pemeriksaan_laboratorium'];

$select = $db->query("SELECT no_faktur,no_rm,no_reg,nama_pasien,petugas_analis,dokter,tanggal FROM hasil_lab WHERE lab_ke_berapa = '$pemeriksaan_laboratorium' AND no_reg = '$no_reg' AND kode_barang = '$kode_barang' AND status = '1'");
...
$selectui = $db->query("SELECT id_sub_header FROM hasil_lab WHERE lab_ke_berapa = '$pemeriksaan_laboratorium' AND no_reg = '$no_reg' AND kode_barang = '$kode_barang' AND status = '1' AND (id_sub_header != 0 OR id_sub_header != '') GROUP BY id_sub_header");
while($trace = mysqli_fetch_array($selectui)){
  ...
  $show = $db->query("SELECT * FROM hasil_lab WHERE lab_ke_berapa = '$pemeriksaan_laboratorium' AND no_reg = '$no_reg' AND kode_barang = '$kode_barang' AND status = '1' AND (id_sub_header != 0 OR id_sub_header != '')");
  ...
  if($face_drop >= 1){
    ...
    $query_hasil_lab = $db->query("SELECT id, nama_pemeriksaan, hasil_pemeriksaan, model_hitung, nilai_normal_lk2, nilai_normal_lk, nilai_normal_pr2, nilai_normal_pr, satuan_nilai_normal, status_pasien FROM hasil_lab WHERE lab_ke_berapa = '$pemeriksaan_laboratorium' AND no_reg = '$no_reg' AND kode_barang = '$kode_barang' AND status = '1' AND id_sub_header = '$id_get' AND (id_sub_header != 0 OR id_sub_header != '')");
...
$query_hasil_lab_tunggal = $db->query("SELECT id, nama_pemeriksaan, hasil_pemeriksaan, model_hitung, nilai_normal_lk2, nilai_normal_lk, nilai_normal_pr2, nilai_normal_pr, satuan_nilai_normal, status_pasien FROM hasil_lab WHERE lab_ke_berapa = '$pemeriksaan_laboratorium' AND no_reg = '$no_reg' AND kode_barang = '$kode_barang' AND status = '1' AND (id_sub_header = 0 OR id_sub_header IS NULL)");
            //menyimpan data sementara yang ada pada $perintah
