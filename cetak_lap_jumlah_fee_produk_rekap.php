...
$total_udah_kelar = 0;
 $query10 = $db->query("SELECT lfp.nama_petugas,lfp.no_faktur,lfp.kode_produk,lfp.nama_produk,lfp.jumlah_fee,lfp.tanggal,lfp.jam ,u.nama FROM tbs_fee_produk lfp LEFT JOIN user u ON lfp.nama_petugas = u.id  LEFT JOIN registrasi r ON lfp.no_reg = r.no_reg WHERE lfp.nama_petugas = '$nama_petugas' AND (lfp.waktu >= '$waktu_dari' AND lfp.waktu <= '$waktu_sampai') AND r.jenis_pasien = 'Rawat Inap' GROUP BY lfp.kode_produk ");
   while($data = mysqli_fetch_array($query10))
            ...
            $nomor = $nomor + 1; 
          $num_row = $db->query("SELECT lfp.nama_produk FROM tbs_fee_produk lfp LEFT JOIN registrasi r ON lfp.no_reg = r.no_reg WHERE lfp.nama_petugas = '$nama_petugas' AND (lfp.waktu >= '$waktu_dari' AND lfp.waktu <= '$waktu_sampai') AND r.jenis_pasien = 'Rawat Inap' AND lfp.kode_produk = '$data[kode_produk]' ");
                $kel = mysqli_num_rows($num_row);

               $SS = $db->query("SELECT SUM(lfp.jumlah_fee) AS jumlah FROM tbs_fee_produk lfp LEFT JOIN registrasi r ON lfp.no_reg = r.no_reg WHERE lfp.nama_petugas = '$nama_petugas' AND (lfp.waktu >= '$waktu_dari' AND lfp.waktu <= '$waktu_sampai') AND r.jenis_pasien = 'Rawat Inap' AND lfp.kode_produk = '$data[kode_produk]'");
                $kel2 = mysqli_fetch_array($SS); 
                $jumlah = $kel2['jumlah'];
 ...
 <table>
  <tbody>
      ...
      <tr><td width="75%"><b>Total Komisi Belum Closing</b></td> <td> &nbsp;:&nbsp;</td> <td> <?php echo rp($total_fee_no_closing); ?> </td></tr>
