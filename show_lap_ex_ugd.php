...
$take_rm = $db->query("SELECT id,no_rm,no_reg,nama_pasien,jenis_kelamin,umur_pasien,alamat_pasien,penjamin,hp_pasien,tanggal FROM registrasi WHERE no_rm = '$no_rm' AND jenis_pasien = 'UGD' AND tanggal >= '$dari_tanggal' AND tanggal <= '$sampai_tanggal'");
...
<?php  
		...
		while($data = mysqli_fetch_array($take_rm))
		{
			...
			<td> <a href='detail_lap_kunjungan_ugd.php?no_rm=".$data['no_rm']."&no_reg=".$data['no_reg']."'  class='btn-floating btn-info btn-small'> 
...
<a href='export_excel_ex_ugd.php?no_rm=<?php echo $no_rm; ?>&dari_tanggal=<?php echo $dari_tanggal; ?>&sampai_tanggal=<?php echo $sampai_tanggal; ?>' type='submit' class='btn btn-warning btn-lg'>Download Excel</a>
