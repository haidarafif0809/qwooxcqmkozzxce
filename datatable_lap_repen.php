...
$query=mysqli_query($conn, $sql) or die("datatable_lap_repen.php: get employees ");
$totalData = mysqli_num_rows($query);
$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.


$sql = "SELECT pel.nama_pelanggan,p.no_faktur_retur,p.id,p.tanggal,p.kode_pelanggan,p.total,p.potongan,p.tax,p.tunai";
$sql.=" FROM retur_penjualan p INNER JOIN pelanggan pel ON p.kode_pelanggan = pel.kode_pelanggan  WHERE 1=1";
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
  $sql.=" AND ( no_faktur_retur LIKE '".$requestData['search']['value']."%' )";
}
$query=mysqli_query($conn, $sql) or die("datatableeee_lap_repen.php: get employees");
