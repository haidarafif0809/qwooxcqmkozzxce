...
//LOGIKA UNTUK MENAMPILKAN HARGA DARI MASING" TRANSAKSI (JUMLAH PRODUK BERTAMBAH)
			if ($row['jenis_transaksi'] == 'Pembelian') {
				...
				$ambil_harga_transfer_stok = $db->query("SELECT harga_unit AS harga_transfer_stok FROM hpp_masuk  WHERE no_faktur = '$row[no_faktur]' AND kode_barang = '$kode_barang' ");
...
//LOGIKA UNTUK MENAMPILKAN HARGA DARI MASING" TRANSAKSI (JUMLAH PRODUK BERKURANG)
			...
			if ($row['jenis_transaksi'] == 'Penjualan') {
				...
				$ambil_harga_transfer_stok = $db->query("SELECT harga AS harga_transfer_stok FROM detail_transfer_stok  WHERE no_faktur = '$row[no_faktur]' AND kode_barang = '$kode_barang' ");
				$data_transfer_stok = mysqli_fetch_array($ambil_harga_transfer_stok);
				$harga_transfer_stok = $data_transfer_stok['harga_transfer_stok'];
