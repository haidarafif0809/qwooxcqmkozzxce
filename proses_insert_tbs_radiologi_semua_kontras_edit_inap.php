...
$query_pemeriksaan_radiologi = $db->query("SELECT kode_pemeriksaan, nama_pemeriksaan, harga_1, kontras FROM pemeriksaan_radiologi WHERE kontras = '1'");
    while ($data_pemeriksaan_radiologi = mysqli_fetch_array($query_pemeriksaan_radiologi))
      {
		...
		$data_tbs_radiologi = mysqli_fetch_array($query_tbs_radiologi);
      	...
      	if ($data_pemeriksaan_radiologi['kode_pemeriksaan'] == $data_tbs_radiologi['kode_barang']) {
      		...
      		$insert_tbs = "INSERT INTO tbs_penjualan_radiologi (kode_barang, nama_barang, jumlah_barang, harga, subtotal, tipe_barang, tanggal, jam, no_reg, kontras, dokter_pengirim, dokter_pelaksana, radiologi, status_pilih,no_pemeriksaan) VALUES ('$data_pemeriksaan_radiologi[kode_pemeriksaan]', '$data_pemeriksaan_radiologi[nama_pemeriksaan]', '$jumlah_barang', $data_pemeriksaan_radiologi[harga_1], $subtotal, '$tipe_barang', '$tanggal_sekarang', '$jam_sekarang', '$no_reg', '$data_pemeriksaan_radiologi[kontras]', '$dokter_pengirim', '$petugas_radiologi', 'Radiologi', 'Pilih Semua','$no_pemeriksaan')";
