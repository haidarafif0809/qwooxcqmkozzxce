...
$perintah3 = $db->query("SELECT * FROM tbs_penjualan WHERE no_faktur = '$no_faktur' AND lab = 'Laboratorium' ");
$data1 = mysqli_num_rows($perintah3);

if ($data1 > 0){

$perintah2 = $db->query("DELETE FROM tbs_penjualan WHERE no_faktur = '$no_faktur' AND lab = 'Laboratorium' ");
}


//menampilkan seluruh data yang ada pada tabel pembelian dalan DB
$perintah = $db->query("SELECT * FROM detail_penjualan WHERE no_faktur = '$no_faktur' AND lab = 'Laboratorium'");
while ($data = mysqli_fetch_array($perintah)){


 $query6 = "INSERT INTO tbs_penjualan (no_faktur, kode_barang,nama_barang,jumlah_barang,satuan,harga,subtotal,tipe_barang,tanggal,jam,potongan,tax,hpp,lab) VALUES ('$no_faktur','$data[kode_barang]','$data[nama_barang]','$data[jumlah_barang]','$data[satuan]','$data[harga]','$data[subtotal]','$data[tipe_produk]','$data[tanggal]','$data[jam]','$data[potongan]','$data[tax]','$data[hpp]','$data[lab]')";
...
$perintah3 = $db->query("SELECT * FROM tbs_fee_produk WHERE no_faktur = '$no_faktur' ");
$data1 = mysqli_num_rows($perintah3);

if ($data1 > 0){

$perintah2 = $db->query("DELETE FROM tbs_fee_produk WHERE no_faktur = '$no_faktur' ");
}


$fee_produk = $db->query("SELECT * FROM laporan_fee_produk WHERE no_faktur = '$no_faktur' ");
...
$insert2 = "INSERT INTO tbs_fee_produk (no_faktur,no_reg,no_rm,nama_petugas,kode_produk,nama_produk,jumlah_fee,tanggal,jam) VALUES ('$data_fee[no_faktur]','$data_fee[no_reg]','$data_fee[no_rm]','$data_fee[nama_petugas]','$data_fee[kode_produk]','$data_fee[nama_produk]','$data_fee[jumlah_fee]','$data_fee[tanggal]','$data_fee[jam]')";
	 ...
	 header ('location:form_edit_penjualan_lab.php?no_faktur='.$no_faktur.'&no_rm='.$no_rm.'&nama_pasien='.$nama_pasien.'');
