...
<div class="form-group"> 
  <select type="text" name="penjamin" id="penjamin_" autocomplete="off" class="form-control" placeholder="Penjamin" >
...
<span id="berdasarkan_penjamin" style="display: none;"><!--span untuk table-->       
    <table id="table_lap_penjamin" class="table table-bordered">
        <thead>
          <th style='background-color: #4CAF50; color:white'> No RM </th>
          <th style='background-color: #4CAF50; color:white'> No REG </th>
          <th style='background-color: #4CAF50; color:white'> Nama Pasien </th>
...
<a href='export_excel_ex_ugd_penjamin.php' type='submit' id='btn_exc_penjamin' class='btn btn-warning btn-lg'>Download Excel</a>
</span>
...
<script type="text/javascript">
     //berdasrkan Penjamin
      ...
      $(document).on('click','#lihat_penj',function(e) {
        ...
        $('#berdasarkan_penjamin').show();
         ...
         $('#table_lap_penjamin').DataTable().destroy();

        var penjamin = $("#penjamin_").val();
        ...
        if (penjamin == '') {
          ...
          var dataTable = $('#table_lap_penjamin').DataTable( {
          "processing": true,
          "serverSide": true,
          "info":     false,
          "language": {
        "emptyTable":     "My Custom Message On Empty Table"
          ...
          "ajax":{
            url :"datatable_lap_kunj_ugd_penjamin.php", // json datasource
             "data": function ( d ) {
                 d.penjamin    = $("#penjamin_").val();
                d.dari_tanggal_penj = $("#dari_tanggal_penj").val();
                d.sampai_tanggal_penj = $("#sampai_tanggal_penj").val();
                // d.custom = $('#myInput').val();
                // etc
            },
                type: "post",  // method  , by default get
            error: function(){  // error handling
              $(".tbody").html("");
              $("#table_lap_penjamin").append('<tbody class="tbody"><tr><th colspan="3"></th></tr></tbody>');
              $("#tableuser_processing").css("display","none");
  ...
  $("#btn_exc_penjamin").attr("href", "export_excel_ex_ugd_penjamin.php?penjamin="+penjamin+"&dari_tanggal="+dari_tanggal+"&sampai_tanggal="+sampai_tanggal+"");
