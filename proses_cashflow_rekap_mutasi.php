...
$sql.=" WHERE DATE(js.waktu_jurnal) = '$tanggal' AND js.kode_akun_jurnal = '$kas' AND dk.kode_akun_jurnal != js.kode_akun_jurnal AND js.kredit != '0' AND js.jenis_transaksi = 'Kas Mutasi' GROUP BY dk.kode_akun_jurnal  ";
...
$sql = "SELECT daf.nama_daftar_akun AS nama_dari_akun ,dk.kode_akun_jurnal AS dari_akun_jurnal , js.jenis_transaksi,da.nama_daftar_akun,js.keterangan_jurnal";
$sql.=" FROM jurnal_trans js LEFT JOIN daftar_akun da ON js.kode_akun_jurnal = da.kode_daftar_akun LEFT JOIN jurnal_trans dk ON js.no_faktur = dk.no_faktur LEFT JOIN daftar_akun daf ON daf.kode_daftar_akun = dk.kode_akun_jurnal ";
$sql.="  WHERE DATE(js.waktu_jurnal) = '$tanggal' AND js.kode_akun_jurnal = '$kas' AND dk.kode_akun_jurnal != js.kode_akun_jurnal AND js.kredit != '0' AND js.jenis_transaksi = 'Kas Mutasi' ";


	$sql.=" AND ( js.jenis_transaksi LIKE '".$requestData['search']['value']."%'";
		$sql.=" OR daf.nama_daftar_akun LIKE '".$requestData['search']['value']."%'";
	$sql.=" OR da.nama_daftar_akun LIKE '".$requestData['search']['value']."%' )  dk.kode_akun_jurnal ";
...
while( $row=mysqli_fetch_array($query) ) {
    ...
    $select = $db->query("SELECT SUM(js.debit) AS keluar FROM jurnal_trans js LEFT JOIN jurnal_trans jkt ON js.no_faktur = jkt.no_faktur WHERE DATE(js.waktu_jurnal) = '$tanggal' AND js.kode_akun_jurnal = '$row[dari_akun_jurnal]' AND js.debit != 0 AND js.jenis_transaksi = 'Kas Mutasi' AND jkt.kode_akun_jurnal = '$kas' ");
