...
$sql.=" FROM jurnal_trans js LEFT JOIN daftar_akun da ON js.kode_akun_jurnal = da.kode_daftar_akun";
$sql.=" WHERE DATE(js.waktu_jurnal) >= '$dari_tanggal' AND DATE(js.waktu_jurnal) <= '$sampai_tanggal' AND js.kode_akun_jurnal = '$kas' AND js.debit != '0' ";
...
while( $row=mysqli_fetch_array($query) ) {
	...
	$select = $db->query("SELECT da.nama_daftar_akun FROM jurnal_trans js LEFT JOIN daftar_akun da ON js.kode_akun_jurnal = da.kode_daftar_akun WHERE DATE(js.waktu_jurnal) >= '$dari_tanggal' AND DATE(js.waktu_jurnal) <= '$sampai_tanggal' AND js.no_faktur = '$row[no_faktur]' AND js.debit = '0' AND js.kredit = js.debit GROUP BY DATE(js.waktu_jurnal)");
	$out = mysqli_fetch_array($select);


	$nestedData[] = $row["tanggal"];
	$nestedData[] = $row["jenis_transaksi"]; //$out["nama_daftar_akun"];
