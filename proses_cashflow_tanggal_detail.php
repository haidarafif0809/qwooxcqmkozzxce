...
$select_keluar = $db->query("SELECT SUM(kredit) AS keluar FROM jurnal_trans WHERE DATE(waktu_jurnal) = '$tanggal' AND kode_akun_jurnal = '$daftar_akun' AND kredit != '0' AND jenis_transaksi != 'Kas Mutasi' GROUP BY no_faktur");
 $outtwo = mysqli_fetch_array($select_keluar);
 $keluar = $outtwo['keluar'];

$select_mutasi = $db->query("SELECT SUM(debit) AS mutasi FROM jurnal_trans WHERE DATE(waktu_jurnal) = '$tanggal' AND kode_akun_jurnal = '$daftar_akun' AND kredit != '0' AND jenis_transaksi = 'Kas Mutasi'");
