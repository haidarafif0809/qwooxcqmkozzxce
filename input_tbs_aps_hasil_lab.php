...
$perintah = $db->query("SELECT kode_jasa,nama_jasa,dokter,analis,harga FROM tbs_aps_penjualan WHERE no_reg = '$no_reg' ");
...
dokter,analis,id_setup_hasil,harga) VALUES ('$satuan_nilai_normal',
...
$perintah = $db->query("SELECT kode_jasa,dokter,analis,harga FROM tbs_aps_penjualan WHERE no_reg = '$no_reg'");
while($data = mysqli_fetch_array($perintah)){
...
if($out_data > 0 AND $datanya != ''){
	...
	model_hitung,no_rm,no_reg,id_pemeriksaan,nilai_normal_lk,nilai_normal_pr,status_pasien,nama_pemeriksaan,id_sub_header,normal_lk2,normal_pr2,kode_barang,dokter,analis,id_setup_hasil,harga) VALUES 
	('$drop[satuan_nilai_normal]','$drop[model_hitung]',
	'$no_rm','$no_reg','$drop[nama_pemeriksaan]','$drop[normal_lk]',
	'$drop[normal_pr]','$jenis_penjualan','$nama_jasa_anak',
	'$id_mother','$drop[normal_lk2]','$drop[normal_pr2]',
	'$kode_barang','$dokter','$analis','$id_setupnya','$harga')";
...
echo '<META HTTP-EQUIV="Refresh" Content="0; URL=hasil_aps_laboratorium.php?no_rm='.$no_rm.'&no_reg='.$no_reg.'&nama='.$nama.'&jenis_penjualan='.$jenis_penjualan.'">';
