...
$no_rm = stringdoang($_GET['kode_pelanggan']);
...
$dokter_pengirim = $db->query("SELECT id FROM user WHERE nama = '$aray[dokter_pengirim]'");
$data_dokter_pengirim = mysqli_fetch_array($dokter_pengirim);
$id_dokter_pengirim = $data_dokter_pengirim['id'];
...
<option ><?php echo $data_penj['perawat'];?></option>
     <?php 
...
<select style="font-size:15px; height:35px" name="dokter_pj" id="dokter_pj" class="form-control chosen" >
  ...
  <?php 
    ...
    while($data01 = mysqli_fetch_array($query01))
    { 
      if ($data01['id'] == $id_dokter_pengirim) {
         echo "<option selected value='".$data01['id'] ."'>".$data01['nama'] ."</option>";
      }
      else{
         echo "<option value='".$data01['id'] ."'>".$data01['nama'] ."</option>";
...
<option ><?php echo $data_penj['apoteker'];?></option>
  <?php 
...
<?php } ?>
                ...
                $perintah = $db->query("SELECT tp.id,tp.kode_barang,tp.satuan,tp.nama_barang,tp.jumlah_barang,tp.harga,tp.subtotal,tp.potongan,tp.tax,s.nama,tp.tanggal,tp.jam,tp.no_reg,tp.tipe_barang FROM tbs_penjualan tp INNER JOIN satuan s ON tp.satuan = s.id WHERE tp.no_reg = '$no_reg' ");
  ...
  <tbody id="tbody">
   ...
   while($data1 = mysqli_fetch_array($utama))      
    ...
    echo "<tr class='tr-kode-". $data1['kode_barang'] ." tr-id-". $data1['id'] ."' data-kode-barang='".$data1['kode_barang']."'>
                ...
                <td style='font-size:15px' align='right'>". rp($data1['harga']) ."</td>
                <td style='font-size:15px' align='right'><span id='text-subtotal-".$data1['id']."'>". rp($data1['subtotal']) ."</span></
td>
                <td style='font-size:15px' align='right'><span id='text-potongan-".$data1['id']."'>". rp($data1['potongan']) ."</span></td>
                <td style='font-size:15px' align='right'><span id='text-tax-".$data1['id']."'>". rp($data1['tax']) ."</span></td>
      ...
      <input style="height:10px;font-size:15px" type="text" name="total" id="total2" class="form-control" placeholder="Total" readonly="" >
        ...
        else
        {
         ...
         $total_akhir1 = $hitung_total + round($hitung_tax) + $biaya_adm;
          ...
          <input type="text" name="potongan" style="height:10px;font-size:15px" id="potongan_penjualan" v class="form-control" placeholder="" autocomplete="off" value="<?php echo $total_potongan; ?>" onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);">
          </div>
...
alert("Silakan Bayar Piutang");
 ...
 $.post("cek_subtotal_penjualan_inap.php",{total:total,no_reg:no_reg,no_faktur:no_faktur,tax:tax,potongan:potongan,biaya_adm:biaya_admin},function(data) {
...
if (info == 1)
{
 ...
 $.post("cek_subtotal_penjualan_inap.php",{total:total,no_reg:no_reg,no_faktur:no_faktur,tax:tax,potongan:potongan,biaya_adm:biaya_admin},function(data) {
  ...
  if (data == "1") {
       ...
       $.post("proses_pay_simpan_ranap.php",{total2:total2,sisa_pembayaran:sisa_pembayaran,kredit:kredit,no_rm:no_rm,tanggal_jt:tanggal_jt,total:total,potongan:potongan,potongan_persen:potongan_persen,tax:tax,cara_bayar:cara_bayar,pembayaran:pembayaran,sisa:sisa,sisa_kredit:sisa_kredit,total_hpp:total_hpp,sales:sales,kode_gudang:kode_gudang,keterangan:keterangan,ber_stok:ber_stok,ppn_input:ppn_input,nama_pasien:nama_pasien,no_reg:no_reg,dokter:dokter,petugas_paramedik:petugas_paramedik,petugas_farmasi:petugas_farmasi,petugas_lain:petugas_lain,penjamin:penjamin,bed:bed,group_bed:group_bed,biaya_admin:biaya_admin,no_faktur:'<?php echo $no_faktur; ?>'},function(info) {
...
$(document).ready(function(){
  $("#batal_penjualan").click(function(){
    ...
    var kode_gudang =  '<?php echo $kode_gudang; ?>'
    var nama_gudang =  '<?php echo $nama_gudang; ?>' 
    var nama_pelanggan =  '<?php echo $nama_pelanggan; ?>'

        window.location.href="batal_penjualan_bayar_ranap.php?no_reg="+no_reg+"&no_faktur="+no_faktur+"&no_rm="+no_rm+"&kode_gudang="+kode_gudang+"&nama_gudang="+nama_gudang+"&nama_pelanggan="+nama_pelanggan+"";
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $.post("cek_tbs_pesanan_ranap.php",{no_reg:no_reg,no_faktur:no_faktur},function(data1){
  
        if (data1 == 1) {
                 $.post("cek_total_bayar_pesanan_barang.php",{no_reg:no_reg,no_faktur:no_faktur},function(data){
                  bersihPemisah(bersihPemisah(bersihPemisah(bersihPemisah(data = data.replace(/\s+/g, '')))));
