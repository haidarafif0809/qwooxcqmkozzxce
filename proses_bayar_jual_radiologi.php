    ...
    $tunai_i = $pembayaran - $total;
          ...
          if ($tunai_i >= 0) 

            {
              ...
              $stmt = $db->prepare("INSERT INTO penjualan (nama, no_faktur, penjamin, petugas_radiologi, kode_pelanggan, total, tanggal, jam, user, sales, status, potongan, tax, sisa, cara_bayar, tunai, status_jual_awal, keterangan, ppn,jenis_penjualan,biaya_admin, no_faktur_jurnal, keterangan_jurnal, dokter) VALUES (?,?,?,?,?,?,?,?,?,?,'Lunas',?,?,?,?,?,'Tunai',?,?,'Radiologi',?,?,?,?)");
              
    // hubungkan "data" dengan prepared statements
              $stmt->bind_param("sssssissssiiisississs",
...
if (!$stmt) 
    ...
    $query3 = $db->query("DELETE FROM tbs_penjualan_radiologi WHERE session_id = '$session_id' AND (no_reg = '' OR no_reg IS NULL)");
